...
Options:  
  ...
  -i, --in=<input_dir>        Input directory [default: {root_path}/data/hmnist.mat].
  -w, --weights=<weight_dir>  Directory with pre-trained model weights from hmnist_encode.
  -o, --out=<output_dir>      Output directory [default: {root_path}/experiments/hmnist_pred/weights/].
...
def train_and_evaluate( cfg: Any, input_dir: str, output_dir: str, load_weight_dir: str):
  ...
  ========== Setup W&B =============
  '''
  exp_name     =  "niso_{}_kop_{}_ld_{}d_hmnist_pred".format( cfg.KERNEL_OP_DIM,
                                                            cfg.LATENT_DIM,
                                                            len(cfg.CONV_ENC_CHANNELS)-1)
  project_name      = cfg.PROJECT_NAME 
  cfg.WORK_DIR      = output_dir 
  cfg.PRE_TRAIN_DIR = load_weight_dir 
  cfg.HMNIST_DIR    = input_dir 
  ...
  train_data, _ = input_pipeline.get_mnist() 
  test_data, num_test = input_pipeline.get_hmnist(cfg.HMNIST_DIR) 
  ...
  for step in tqdm( range(initial_step, stop_at) ):
    ...
    if step % cfg.EVAL_EVERY == 0 or is_last_step:
      ...
      for batch in tqdm(test_data):
        ...
        state, emetrics_update = p_eval_step(x=batchIm[None, ...], labels=labels[None, ...], state=state)
