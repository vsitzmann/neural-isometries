...
Options:
  ...
  -o, --out=<output_dir>  Output directory [default: {root_path}/experiments/hmnist_encode/weights/].
...
def train_and_evaluate( cfg: Any, output_dir: str ):
  ...
  train_data, test_data = input_pipeline.get_mnist() 
  ...
  train_data = train_data.map( dl.pad_images(cfg.INPUT_SIZE), num_parallel_calls=tf.data.AUTOTUNE )
  ...
  test_data = test_data.map( dl.pad_images(cfg.INPUT_SIZE), num_parallel_calls=tf.data.AUTOTUNE )
  ...
  xform = xforms.Homographies()
  
  print( "Done..." )

  #Create models
  print( "Initializing models..." )
  encoder, decoder, kernel, optimizer, xform_key, state = create_train_state( cfg, (batch_size, *input_size, 1) )
  print( "Done..." )
  ...
  for step in tqdm( range(initial_step, stop_at) ):
    ...
    batch = xform.draw_and_apply( batch, batch_key)
