...
Options:
  ...
  -o, --out=<output_dir>  Output directory [default: {root_path}/experiments/pose_encode/weights/].
...
@functools.partial(jax.jit, static_argnames=["max_step"]) 
def pair_sampler(image, mask, key, max_step=None):

  
  frame_key, step_key = jax.random.split(key, 2)
 
  f_keys = jax.random.split(frame_key, image.shape[0])
  
  if max_step is not None:
 
    max_bound = jnp.argmax(jnp.arange(mask.shape[-1]) * mask) 
    
    max_bstep = jnp.minimum(max_bound // 2, max_step)
    
    indices = mask * jnp.arange(mask.shape[-1])[None, :] + 1e8 * (1.0 - mask) 

    dist = jnp.abs(indices[..., None] - indices[..., None, :])

    nearest = jnp.argsort(dist, axis=-1)[..., :(2 * max_step + 1)]
    
    f0 = jax.vmap(jax.random.choice, (0, None, None, None, 0, None), 0)(f_keys, mask.shape[-1], (), False, mask, 0)

    f1 = jax.random.randint(step_key, (image.shape[0], ),  minval=0, maxval=2*max_step)
    f1 = jnp.minimum(2*max_bstep, f1)
    f1 = nearest[jnp.arange(image.shape[0]), f0, f1]

  else:
    s_keys = jax.random.split(step_key, image.shape[0])
    f0 = jax.vmap(jax.random.choice, (0, None, None, None, 0, None), 0)(f_keys, mask.shape[-1], (), False, mask, 0)
    f1 = jax.vmap(jax.random.choice, (0, None, None, None, 0, None), 0)(s_keys, mask.shape[-1], (), False, mask, 0)
  
  return f0, f1 

@functools.partial(jax.jit, static_argnames=["max_step"]) 
def double_sampler(image, mask, key, max_step=None): 

  
  f0, f1 = pair_sampler(image, mask, key, max_step)

  x = image[jnp.arange(image.shape[0]), f0, ...]
  gx = image[jnp.arange(image.shape[0]), f1, ...]

  x = jnp.concatenate((x[:, None, ...], gx[:, None, ...]), axis=1)
 
  return jnp.reshape(x, (-1, image.shape[2], image.shape[3], image.shape[4]))
...
def viz_results( cfg, z, batch, x_out, tauOmega, Omega, mode="train", std_factor=2.5, image_dir=None):
  ...
  [x, gx] = jnp.split(jnp.reshape(batch, (-1, 2, batch.shape[1], batch.shape[2], batch.shape[3])), 2, axis=1)
  [x0, gx0] = jnp.split(jnp.reshape(x_out, (-1, 2, batch.shape[1], batch.shape[2], batch.shape[3])), 2, axis=1)

  xIm = jax.image.resize( x[:, 0, ...], (x.shape[0], 256, 256, x.shape[-1]), method="nearest" )
  gxIm = jax.image.resize(gx[:, 0, ...], (gx.shape[0], 256, 256, gx.shape[-1]), method="nearest")
  
  xIm = np.asarray(255.0 * jnp.clip(xIm, 0, 1)).astype(np.uint8)
  gxIm = np.asarray(255.0 * jnp.clip(gxIm, 0, 1)).astype(np.uint8)
  
  
  [z_p, gz_p] = jnp.split(z_p, 2, axis=1)
  [x0_p, gx0_p] = jnp.split(jnp.reshape(x_out_p, (-1, 2, batch.shape[1], batch.shape[2], batch.shape[3])), 2, axis=1)
  ...
  imPhi_wandb   = [wandb.Image(O_clr, caption="Operator")]
...
def train_and_evaluate( cfg: Any, input_dir: str, output_dir: str ):
  ...
  exp_name     =  "niso_{}_kopd_{}_ld_{}d_pose_encode".format(cfg.KERNEL_OP_DIM, cfg.LATENT_DIM, len(cfg.CONV_ENC_CHANNELS)-1)  
  ...
  train_data, test_data = input_pipeline.get_seq_dataset( input_dir, compression_type=cfg.DATA_COMPRESSION_TYPE)
  ...
  train_data = train_data.shuffle( buffer_size=100, reshuffle_each_iteration=True )
  ...
  test_data = test_data.batch( batch_size, drop_remainder=True, num_parallel_calls=tf.data.AUTOTUNE )
  ...
  encoder, decoder, kernel, optimizer, xform_key, state = create_train_state( cfg, (batch_size, *input_size, 3) )
  print( "Done..." )
  ...
  for step in tqdm( range(initial_step, stop_at) ):
    ...
    tmask  = jnp.asarray( tmask.numpy() )

    xform_key, batch_key = jax.random.split( xform_key )
    batch                = sample_fn( batch, tmask, batch_key, max_step )
    ...
    if step == initial_step:
      ...
      batchN = 0.5 * (batchN + 1.0)

      im0, imX = np.split( np.asarray(batchN * 255.0).astype(np.uint8), 2, axis=1 )
      im0      = np.concatenate( (im0, imX), axis=2 )
      
      batch_im = []
      
      for l in range( 1 ): #range(im0.shape[0]):
        batch_im.append( wandb.Image(im0[l, ...], caption="batch_input_ex") )
    ...
    if step % cfg.EVAL_EVERY == 0 or is_last_step:
      ...
      for j in range( cfg.NUM_EVAL_STEPS ):
        ...
        emask  = jnp.asarray( ex["frame_mask"].numpy() )
        
          
        xform_key, batch_key = jax.random.split( xform_key )
        batch                = sample_fn( batch, emask, batch_key, max_step )
